...
typedef struct Modifiers {
	...
	explicit Modifiers(SDL_Event& event) {
		...
		shift = (mod & SDL_KMOD_LSHIFT) || (mod & SDL_KMOD_RSHIFT);
		ctrl = (mod & SDL_KMOD_LCTRL) || (mod & SDL_KMOD_RCTRL);
		alt = (mod & SDL_KMOD_LALT) || (mod & SDL_KMOD_RALT);
		num_lock = !(mod & SDL_KMOD_NUM);
		caps_lock = static_cast<bool>(mod & SDL_KMOD_CAPS);
...
typedef struct KeyCodes {
	...
	KeyCodes(int keyCode, int charCode) : key_code(keyCode), char_code(charCode) {
	}
...
static KeyCodes getFunctionKeyCodes(SDL_Keycode code) {
	return {112 + ((int)code - 1073741882), DEFAULT_CHAR_CODE};
}
...
void handleKeyEvent(SDL_Event& event, CefRefPtr<CefBrowser> browser) {
	...
	if (keyCodes.key_code == 0) {
		keyCodes.key_code = event.key.key;
	}

	/** Fire key events to CEF **/
	if (event.key.down || event.type == SDL_EVENT_TEXT_INPUT) {
		// onkeydown
