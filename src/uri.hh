...
public:
	...
	uri(std::map<component, std::string> const& components, scheme_category category,
			bool rooted_path, query_argument_separator separator = query_argument_separator::ampersand)
		...
		if (category == scheme_category::Hierarchical) {
			if (components.count(component::Content)) {
				throw std::invalid_argument(
						"The content component is only for use in non-hierarchical URIs.");
			}
			...
			if (has_username && has_password) {
				...
				throw std::invalid_argument(
						"If a username or password is supplied, both must be provided.");
			}
			...
			if (components.count(component::Path)) {
				...
				throw std::invalid_argument(
						"A path is required on a hierarchical URI, even an empty path.");
			}
		} else {
	...
	uri(uri const& other, std::map<component, std::string> const& replacements)
			: m_category(other.m_category), m_path_is_rooted(other.m_path_is_rooted),
				m_separator(other.m_separator) {
		m_scheme = (replacements.count(component::Scheme)) ? replacements.at(component::Scheme)
																											 : other.m_scheme;

		m_username = (replacements.count(component::Username)) ? replacements.at(component::Username)
																													 : other.m_username;

		m_password = (replacements.count(component::Password)) ? replacements.at(component::Password)
																													 : other.m_password;

		m_host =
				(replacements.count(component::Host)) ? replacements.at(component::Host) : other.m_host;

		m_port = (replacements.count(component::Port)) ? std::stoul(replacements.at(component::Port))
																									 : other.m_port;

		m_path =
				(replacements.count(component::Path)) ? replacements.at(component::Path) : other.m_path;

		m_content = (replacements.count(component::Content)) ? replacements.at(component::Content)
																												 : other.m_content;

		m_query =
				(replacements.count(component::Query)) ? replacements.at(component::Query) : other.m_query;

		m_fragment = (replacements.count(component::Fragment)) ? replacements.at(component::Fragment)
																													 : other.m_fragment;
	}
...
private:
	...
	std::string::const_iterator parse_scheme(std::string const& uri_text,
		...
		while ((scheme_end != uri_text.end()) && (*scheme_end != ':')) {
			if (!(std::isalnum(*scheme_end) || (*scheme_end == '-') || (*scheme_end == '+') ||
						(*scheme_end == '.'))) {
				throw std::invalid_argument(
	...
	std::string::const_iterator parse_host(std::string const& uri_text, std::string const& content,
	...
	std::string::const_iterator parse_port(std::string const& uri_text, std::string const& content,
